TEAMS = ['Башня', 'ForREST', 'Bionicle', 'Alliance', '8bit']
ACTION_TYPES = {
    'up_priority': 'up_priority',
    'down_priority': 'down_priority',
    'suggest_deadline': 'suggest_deadline'
}
OUT_MESSAGES = {
    'wrong_input': 'Нет такого сценария.\nИ команды у нас такой нет.\n🤔',
    'no_scenarios_for_your_team': 'У вашей команды нет сценариев',
    'statuses': f'*СТАТУСЫ*\n\n*backlog* — не брали в работу,\n\n*in progress* — в работе,\n\n*home check* — '
                f'внутреннее ревью,\n\n*☑️ ready for BRF* — готово к передаче в разработку,\n\n*✅ ready for code* — '
                f'передано в разработку,\n\n*code* — в разработке,\n\n*design review* — на ревью\n\n ',
    'unregistered_user': 'Незарегистрированный пользователь',
    'help': '/statuslegend — подсказка по статусам готовности макетов',

}

USERS = [
    [182441987, 'Dmitriy', 'Yukhin'],
    [78828759, 'Stas', 'V'],
    [147534504, 'Dmitry', 'Andreev'],
    [367196652, 'Stanislav', 'Koshevoy'],
    [204133455, 'Artem', 'Mukhin'],
    [180030971, 'Denis', 'Fomin'],
    [238519111, 'Ksenia', 'Chesnokova'],
    [128041559, 'Dmitry', 'Nösov'],
    [755639858, 'Светлана', 'П.'],
    [1083593052, 'Dimati', '013'],
    [1029460206, 'Евгений', 'Шевелев'],
]
